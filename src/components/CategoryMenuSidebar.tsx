...
function CategoryMenuSidebar() {
  ...
  const categories = [
    ...
    {
      name: "봉사/사회참여",
      icon: "/images/volunteer_dark.svg",
      sub: ["복지/나눔", "캠페인"],
    },
  ];
  ...
  return (
    <motion.aside
      ...
      className="fixed top-[120px] left-[calc(50%-812px+24px)] w-[235px] z-50 hidden lg:block"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-white font-bold text-[28px] leading-normal">
            모임생성
          </span>
          <img
      ...
      {/* 카테고리 */}
      <nav className="mt-5">
        <h3 className="mb-2 mt-3 text-[20px] font-bold text-gray-700">
          카테고리
        </h3>
        <ul className="space-y-1">
          {categories.map((cat) => {
            const isActiveMain =
              activeMain === cat.name ||
              (cat.sub && cat.sub.includes(activeSub));
            ...
            return (
              <li key={cat.name}>
                ...
                >
                  <img
                    src={
                      isActiveMain ? cat.icon.replace("_dark", "") : cat.icon
                    }
                    alt={cat.name}
                    className="w-4 h-4"
                  />

...
function ScheduleModal({
  ...
  const handleSubmit = () => {
    ...
    if (form.startDate.isBefore(today)) {
      setErrorMessage("오늘보다 이전 날짜는 선택할 수 없습니다.");
      return;
    }
  ...
  return (
    <Modal
      ...
      title={
        <h3 className="text-center text-[#0762E5] text-[25px] font-bold m-3">
          {titleText}
        </h3>
      }
    >
      <div className="flex flex-col gap-6 px-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <DatePicker
              ...
              disabledDate={(current) =>
                (form.startDate && current < form.startDate.startOf("day")) ||
                current < dayjs().startOf("day")
              }
            />
          </div>
          {/* 에러 메시지 표시 */}
          {errorMessage && <p className=" text-red-500 ml-1">{errorMessage}</p>}
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xs font-medium ${
                form.title.length >= 35 ? "text-red-500" : "text-gray-400"
              }`}
            >
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xs font-medium ${
                form.location.length >= 20 ? "text-red-500" : "text-gray-400"
              }`}
            >

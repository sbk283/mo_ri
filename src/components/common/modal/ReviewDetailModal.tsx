...
export type ReviewDetail = {
  ...
  tags: string[];
  empathy: number;
  creator_id: string; // 닉네임
  creator_avatar?: string; // ✅ 작성자 아바타(추가)
};
...
export default function ReviewDetailModal({ open, review, onClose, onEmpathy }: Props) {
  ...
  const handleEmpathy = () => {
    const alreadyClicked = false; // 실제 상태는 상위 컴포넌트에서 관리
    if (alreadyClicked) {
      setOpenError(true);
      setTimeout(() => setOpenError(false), 1200);
      return;
    }
    onEmpathy?.(review.id);
  };

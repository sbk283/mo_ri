...
import { today, resolveStorageUrl, getFirstImageUrl, NO_IMAGE } from '../../lib/contentUtils';
import {
  fetchDailyList,
  createDaily,
  updateDaily,
...
export default function GroupDailyContent({
  ...
  // 좋아요 상태/실시간 훅
  const { likeCountMap, likedByMe, prime, toggleLike, setupRealtime, teardownRealtime } =
    useDailyLikes();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setItems(mapped);
      setProfileByPostId(
        Object.fromEntries(data.map((r: any) => [r.post_id, r.user_profiles?.avatar_url ?? null])),
      );
  ...
  const openDetail = (id: number) => {
    ...
    if (post) {
      ...
      void bumpDailyView(post.postId).then(() =>
        setItems(prev =>
          prev.map(it =>
            it.postId === post.postId ? { ...it, viewCount: (it.viewCount ?? 0) + 1 } : it,
          ),
        ),
      );
    }
  ...
  const handleCreateSave = async (next: Daily) => {
    if (!groupId) return;
    const { data: inserted, error } = await createDaily(groupId, next.title, next.content);
    if (error || !inserted) {
  ...
  const handleUpdateSave = async (next: DailyWithPostId) => {
    ...
    const { error } = await updateDaily(next.postId, uid, next.title, next.content);
    if (error) {
              ...
              <>
                <div className="grid grid-cols-3 auto-rows-fr gap-3 py-6">
                  {pageItems.map(daily => {
                    ...
                    const thumb = resolveStorageUrl(daily.imageUrl) ?? daily.imageUrl ?? NO_IMAGE;
          ...
          >
            {selectedDaily ? (
              <GroupDailyDetailView
                ...
                likeCount={likeCountMap[selectedDaily.postId] ?? selectedDaily.likedCount ?? 0}
                liked={!!likedByMe[selectedDaily.postId]}
                onToggleLike={() => toggleLike(selectedDaily.postId, currentUserId)}

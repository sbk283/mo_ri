...
export default function GroupContentBox() {
  ...
  return (
    <div className="w-[1024px] mx-auto space-y-4">
      {groups.map(g => {
        const cover = g.image_urls?.[0] ?? '/images/placeholder.jpg';
        const to = `/groupcontent/${g.group_id}`;
        const start = new Date(g.group_start_day);
        const daysLeft = Math.ceil((start.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        const isBeforeOpen = !Number.isNaN(daysLeft) && daysLeft > 0;
        const memberCnt = memberCountMap[g.group_id] ?? 0;
        const capacity = g.group_capacity ?? 0;

        return (
          <Link
            key={g.group_id}
            to={to}
            className="w-[1024px] h-[123px] border rounded-[5px] border-[#acacac] p-[10px] relative flex hover:shadow transition"
          >
            {isBeforeOpen && (
            ...
            <div className="px-4 flex flex-col justify-between flex-1">
              <div className="flex items-center gap-3">
                <p className="text-lg font-bold line-clamp-1">{g.group_title}</p>
              </div>
              <div className="text-sm text-[#111] line-clamp-2">
                {g.group_short_intro ?? '소개 글이 아직 없어요.'}
              </div>
